...
class Lsm6ds3 {
 ...
 private:
  ...
  [[nodiscard]] uint8_t read_reg(uint8_t reg);
  [[nodiscard]] std::array<int16_t, 3> read_3_axis_raw(uint8_t start_reg);

  [[nodiscard]] double accel_lsb_to_mps2() const;
  [[nodiscard]] double gyro_lsb_to_rads() const;
